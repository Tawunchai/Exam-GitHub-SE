...
import { ListReports, GetAnimalByHealth, DeleteReportByID } from "../../../services/https/index";
...
const Report = () => {
  ...
  const handleOk = async () => {
    ...
    try {
      ...
      if (res) {
        setReports((prevReports) => prevReports.filter((report) => report.ID !== deleteId)); // Update state to remove deleted report
        setReportCount((prevCount) => prevCount - 1); // Update count
        setOpen(false);
        messageApi.open({
          type: "success",
