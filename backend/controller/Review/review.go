...
func CreateReview(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"message": "Review created successfully","id": review.ID, })
}
...
func ListBookingsByUserID(c *gin.Context) {
	...
	for _, booking := range bookings {
		...
		for _, paymentTicket := range paymentTickets {
			if paymentTicket.PaymentStatus == "Complete" {
...
func UpdateReview(c *gin.Context) {
	...
	if rating := c.PostForm("Rating"); rating != "" {
		if r, err := strconv.ParseUint(rating, 10, 32); err == nil {
